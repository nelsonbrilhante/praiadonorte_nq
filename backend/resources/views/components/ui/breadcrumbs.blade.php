...
@if(count($items) > 0 || $showHome)
    <nav aria-label="Breadcrumb" {{ $attributes->merge(['class' => 'py-3 border-b border-ocean/10 bg-muted/20 dark:bg-white/[0.02]']) }}>
        <div class="container mx-auto px-4">
            <ol class="flex flex-wrap items-center gap-1.5 text-sm text-muted-foreground">
                @if($showHome)
                    <li class="flex items-center gap-1.5">
                        <a href="{{ LaravelLocalization::localizeURL('/') }}" class="flex items-center gap-1 hover:text-ocean transition-colors">
                ...
                @foreach($items as $index => $item)
                    <li class="flex items-center gap-1.5">
                        ...
                        @else
                            <a href="{{ $item['href'] }}" class="hover:text-ocean transition-colors">
                                {{ $item['label'] }}
                            </a>
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="text-muted-foreground/50">
                                <path d="m9 18 6-6-6-6"/>
