...
export default async function NazareQualificaSobrePage({ params }: Props) {
  const { locale } = await params;
  setRequestLocale(locale);

  const pagina = await fetchPagina('nazare-qualifica', 'sobre');
      ...
      <section className="py-12">
        <div className="container mx-auto px-4">
          <div className="mx-auto max-w-4xl">
            <article
              className="prose prose-lg max-w-none dark:prose-invert prose-headings:text-institutional prose-a:text-institutional prose-li:marker:text-institutional prose-strong:text-institutional"
            ...
            <div className="mt-12 rounded-lg bg-institutional/10 p-8 text-center">
              <h3 className="mb-4 text-2xl font-bold">Conheça os nossos serviços</h3>
              <div className="flex flex-wrap justify-center gap-4">
                <Button asChild className="bg-institutional hover:bg-institutional/90">
                  <Link href={`/${locale}/nazare-qualifica/servicos`}>Ver Serviços</Link>
                </Button>
