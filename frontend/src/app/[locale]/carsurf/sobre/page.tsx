...
export default async function CarsurfSobrePage({ params }: Props) {
  const { locale } = await params;
  setRequestLocale(locale);

  const pagina = await fetchPagina('carsurf', 'sobre');
      ...
      <section className="py-12">
        <div className="container mx-auto px-4">
          <div className="mx-auto max-w-4xl">
            <article
              className="prose prose-lg max-w-none dark:prose-invert prose-headings:text-performance prose-a:text-performance"
            ...
            <div className="mt-12 rounded-lg bg-performance/10 p-8 text-center">
              <h3 className="mb-4 text-2xl font-bold">Interessado nos nossos programas?</h3>
              <div className="flex flex-wrap justify-center gap-4">
                <Button asChild className="bg-performance hover:bg-performance/90">
                  <Link href={`/${locale}/carsurf/programas`}>Ver Programas</Link>
                </Button>
